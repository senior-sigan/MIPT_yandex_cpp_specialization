...
class BusStations {
 ...
 public:
  ...
  void buses_for_stop(string stop_name) {
    ...
    for (auto bus : stops[stop_name]) {
      cout << bus;
      if (++i < stops.size()) cout << " ";
    }
    cout << endl;
  }
  void stops_for_bus(string bus_name) {
  ...
  void new_bus(string bus, vector<string> bus_stops) {
...
void handle(BusStations& busStations) {
  string op;
  cin >> op;

  if (op == "ALL_BUSES") {
    busStations.all_buses();
  } else if (op == "ALL_STOPS") {
    busStations.all_stops();
  } else if (op == "STOPS_FOR_BUS") {
    string bus_name;
    cin >> bus_name;
    busStations.stops_for_bus(bus_name);
  } else if (op == "NEW_BUS") {
    string bus_name;
    int stops_count;
    ...
    busStations.new_bus(bus_name, stops);
  } else if (op == "BUSES_FOR_STOP") {
    string stop_name;
    cin >> stop_name;
    busStations.buses_for_stop(stop_name);
  }
}
